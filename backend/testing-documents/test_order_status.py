...
def test_update_status_locked_delivered():
    ...
    with pytest.raises(ValueError) as excinfo:
        update_order_status(order, "CANCELLED", fake_customer) #try to change the status to cancelled, should raise error because the order is locked
...
def test_forward_success():
    ...
    updated_order = update_order_status(order, "PREPARING", fake_manager) #try to change the status to preparing, should work because the order is not locked
...
def test_invalid_transition():
    order = create_dummy_order(status="PENDING") #make the status pending (not locked)
    with pytest.raises(ValueError) as excinfo:
        update_order_status(order, "DELIVERED", fake_manager) #try to change the status to delivered, should raise error because you cant do this
    assert "Invalid status transition" in str(excinfo.value)

def test_customer_cancel_pending():
    order = create_dummy_order(status="PENDING") #make the status pending (not locked)
    updated_order = update_order_status(order, "CANCELLED", fake_customer) 
    assert updated_order.status == "CANCELLED" 
def test_customer_cancel_preparing():
    order = create_dummy_order(status="PREPARING") #make the status preparing (not locked)
    with pytest.raises(ValueError) as excinfo:
        update_order_status(order, "CANCELLED", fake_customer) #try to change the status to cancelled, should raise error because you can only cancel pending orders
    assert "Order cannot be cancelled at this stage" in str(excinfo.value)

def test_customer_cannot_accept_order():
    order = create_dummy_order(status="PENDING") #make the status pending (not locked)
    with pytest.raises(ValueError) as excinfo:
        update_order_status(order, "PREPARING", fake_customer) #try to change the status to preparing, should raise error because customers can only cancel orders
...
def test_unauthorized_user():
    ...
    with pytest.raises(ValueError) as excinfo:
        update_order_status(order, "PREPARING", fake_user) #try to change the status to preparing, should raise error because the user class is unauthorized
    assert "Unauthorized user class" in str(excinfo.value)

def test_manager_can_cancel_preparing():
    order = create_dummy_order(status="PREPARING") #make the status preparing (not locked)
    updated_order = update_order_status(order, "CANCELLED", fake_manager) 
    assert updated_order.status == "CANCELLED" #should work because restaurant managers can cancel orders at any stage